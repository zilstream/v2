...
export const Route = createFileRoute("/tx/$hash")({
  loader: async ({ params }) => {
    try {
      const tx = await fetchTransactionByHash(params.hash);
      return { tx };
    } catch {
      throw notFound();
    }
  },
  pendingComponent: TransactionLoading,
...
function TransactionDetailPage() {
  ...
  return (
    <div className="flex w-full flex-col gap-4 p-3 md:gap-6 md:p-6">
        ...
        <CardContent className="space-y-4">
          ...
          <DetailRow
            ...
            value={
              <Link
                to="/block/$number"
                params={{ number: tx.blockNumber.toString() }}
          ...
          <DetailRow
            ...
            value={getTransactionType(tx.transactionType)}
          />
        </CardContent>
      </Card>
                      ...
                      <div className="flex flex-col gap-1 sm:flex-row sm:gap-4">
                        ...
                        <dd className="flex-1 space-y-1 text-sm">
                          {event.topics.map((topic, i) => (
                            <div
                              // biome-ignore lint/suspicious/noArrayIndexKey: topic list is static per event
                              key={i}
      ...
      <Link to="/txs" className="text-sm text-primary hover:underline">
...
function getTransactionType(type: number) {
  if (type >= 1000) return "Zilliqa Legacy";
