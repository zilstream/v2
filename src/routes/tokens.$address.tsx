...
export const Route = createFileRoute("/tokens/$address")({
  loader: async ({ params }) => {
    const [token, pairsResponse, chartData] = await Promise.all([
      fetchTokenByAddress(params.address),
      fetchTokenPairs(params.address),
      fetchTokenChart(params.address),
    ]);
    return { token, pairs: pairsResponse.data, chartData };
  },
  component: TokenDetailPage,
});

function TokenDetailPage() {
  const { token, pairs, chartData } = Route.useLoaderData();
